...
class RunVisualiser:
    ...
    def update_plot (self,):
        ...
        if self.draw_all_progs_fit:
            for prog in run_logger.programs_epoch[run_logger.notkept]:
                try:
                    ...
                    curr_ax.plot(x_plot_cpu, y_plot, color='w', alpha=0.05, linestyle='solid')
                except:
        ...
        curr_ax.set_ylabel("Shear stress")
        curr_ax.set_xlabel("Time")
        curr_ax.legend(custom_lines, ['Overall Best', 'Best of epoch', 'Train'], loc=LEGEND_LOC)
        fig = curr_ax.get_figure()
        fig.savefig("rheology.pdf", format="pdf")
