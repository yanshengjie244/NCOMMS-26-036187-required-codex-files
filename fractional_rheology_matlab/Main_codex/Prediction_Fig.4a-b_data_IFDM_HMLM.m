...
T_SPLIT = 80; % Time point separating fitting and prediction regions
...
MARKER_SIZE = 1520;     % Significantly enlarge experimental data points
...
% --- Line width settings ---
LINE_WIDTH_MAIN = 15.0; % Main line width (kept visually strong for this figure size)

%% -------------------- Import data --------------------
% Make sure the data file is in the current folder.
r = xlsread("Fig.4a-b_data.xlsx", "A3:A80");  % Time (t), column vector
ss = xlsread("Fig.4a-b_data.xlsx", "B3:B80"); % Shear Stress (tau), column vector
...
a0_vmlm = [0.1,0.1,0.7,0.01,0.5,0.1,0.5];
p_vmlm = lsqcurvefit(@aefun, a0_vmlm, r_fit, ss_fit, [0 0 0 0 0 0 0], [Inf Inf Inf Inf Inf Inf Inf], options);
...
lgd = legend([ h1_fit, h2_fit], { 'VMLM', 'IFDM'}, ...
...
function ae_out = aefun(a, t)
    ae_out = a(1)+(a(7)+(a(2)-a(7)).*a(3).^(exp(-t.*a(4)).*log(a(5)./t))).*t.^a(6).*100
end

function c_out = cfun(c, t)
    a = c(4) + (c(5)-c(4))./(1 + exp(-c(6).*(t - c(7))));
    c_out = c(1) + (c(2).*c(3).^a .* 100 .* t.^(1-a) ./ gamma(2-a));
end
